...
@Component({
  ...
  styleUrl: './bookings-summary.component.css',
})
export class BookingsSummaryComponent {
  ...
  updateBookings(from: string, to: string) {
    this.bookingService
      .getBookingsByTypeAndRange(RESOURCE_TYPE_BOOKING, from, to)
      .subscribe({
        next: (bookings) => {
